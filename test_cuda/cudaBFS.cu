...
UL *do_bfs_cuda(UL source, csrdata *csrgraph, csrdata *csrgraph_gpu, double *cudatime, int iter)
{
...
UL *traverse_parallel(UL *edges, UL nedges, UL nvertices, UL root, int randsource, int seed)
{
    ...
    for(i=0; i<10; i++) {
        dist = do_bfs_cuda(root, &csrgraph, &csrgraph_gpu, &cudatime, i);
        tottime += cudatime;
    }
    cudatime = tottime / 10;
