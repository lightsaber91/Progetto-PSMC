...
UL *do_bfs_omp(UL source, csrdata *csrg, int thread, int counter) {

    UL *q, ql, qs, i, start, end, *dist, d, U, V, s, e, j;
    char *visited;

    // Imposto il numero di thread a mano o automaticamente
    if(thread > 0 && thread <= 24) {
        omp_set_num_threads(thread);
        if(!counter) printf("Numero Threads: %d\n", thread);
    }
    else {
...
UL *traverse_parallel(UL *edges, UL nedges, UL nvertices, UL root, int randsource, int seed, int thread) {
    ...
    while(thread < 33) {
        ...
        for(i = 0; i < 10; i++) {
            START_TIMER(begin)
            dist = do_bfs_omp(root, &csrgraph, thread, i);
            END_TIMER(end);
